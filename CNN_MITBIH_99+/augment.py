...
def cls_prob(ar):
    ...
    pred=np.argmax(ar,axis=1)
    for i in range (len(pred)):
        prbabl=np.append(prbabl,ar[i,pred[i]])
    return(prbabl)

def valuation(P,tr,trlt,grade=1,k=8):                               #محاسبه ی ارزش هر نمونه ی اقلیت (مثبت) جهت تکثیر
    importnc=[]
...
def ClsPrb(Ag):
    model=load_model('CNN16.h5')
    predict_x=model.predict(Ag)                        # Function 1
    return(cls_prob(predict_x))                        # function from augment.py to remove 0 index predictions
...
def dstrbtn_indx(ttl_ln,nd_ln,dstr):#(total_data_length,needed_data_length,distribution)
    ttl_ln=int(ttl_ln)
    nd_ln=int(nd_ln)
    ttl_ln=ttl_ln-2
    indxs=[]
    ...
    if dstr==1 or nd_ln>.7*ttl_ln:      #sequencial=select Respectively from first of list
        ...
        while len(indxs)<(nd_ln):       #triangular(left, mode, right, size=None)
            ...
            indxs_ttl=np.delete(indxs_ttl,nw_indx,axis=0)
            ttl_ln=ttl_ln-1
        ...
        while len(indxs)<(nd_ln):         #beta(a, b, size=None)
            nw_indx=int(np.random.normal(0.5, .1)*ttl_ln)
            indxs.append(indxs_ttl[nw_indx])
            indxs_ttl=np.delete(indxs_ttl,nw_indx,axis=0)
            ttl_ln=ttl_ln-1
...
def dstrbtn_indx2(ttl_ln,nd_ln,dstr):#(data,total_data_length,needed_data_length,distribution)
    ...
    if dstr==1 or nd_ln>.7*ttl_ln :     #sequencial=select Respectively from first of list
        ...
        indxs=np.random.randint(0,ttl_ln,nd_ln)
    elif dstr==6 :                       #error: data in range second half was not selected
...
def SrtAg(mthd,crtria,i,nmbr,invrs=1):           #import data from gan generated data
    import glob
    ...
    data=np.array(data)
    if crtria==1:
        dstr=4                                             #1-sequencial 2-linear 3-beta 4-exponential 5-unfrm 6-normal
        srt=-2                                             # -1==ClsPrb  -2==Marginality                
    elif crtria==2:
        dstr=2                                             #1-sequencial 2-linear 3-beta 4-exponential 5-unfrm 6-normal
        srt=-2                                             # -1==ClsPrb  -2==Marginality                
    if crtria==3:
        dstr=6                                             #1-sequencial 2-linear 3-beta 4-exponential 5-unfrm 6-normal
        srt=-1                                             # -1==ClsPrb  -2==Marginality                
    elif crtria==4:                                        #!!!!!!!!!!  just test it for GAN Data !!!!!!!!!!!!!
        dstr=6                                             #1-sequencial 2-linear 3-beta 4-exponential 5-unfrm 6-normal
        srt=-2                                             # -1==ClsPrb  -2==Marginality                
    if invrs==0 :
        dt=data[data[:,srt].argsort()]                     # sort base of Criteria (Quaity min>max)
    else:
        dt=data[(-data[:,srt]).argsort()]                  # sort bae of Criteria  #change min>max to max>min Criteria sort
    ...
    return alldata[:nmbr,1:-2] #(Remove end clmns: mthd,clstr,epoch, Criterias) Criteria=2>[:nmbr,1:-2],Criteria=5>[:nmbr,1:-5] 
