...
def GAN_Gnrt(wndws):
    num_clses=len(set(wndws[:,-1])          #clsses are end column, classes_labels>=1
    for z in range (1,num_clses+1):
        cls=z
        ...
        all_smpls=2             #1/2/3 are 50,000/100,000/200,000 samples for train GAN (300/600 epoch)
        snstvty=2               #1or2 ara low or high sensitivity to detect a beat
        print("\n\n\n Class ", str(cls)," Shape = ", np.shape(vars()['ecg'+str(cls)]))
        noise_len=90
        ...
        print('for class ', cls,'number of windows is = ',np.shape(wndws[0]))
         ...
         #EPOCHS = all_smpls*int(50000/len(wndws))#(صد یا پنجاه هزار نمونه برای آموزش شبکه(به قسمت کلاستر رفته
        x_train=np.array(wndws[:,:-1]) #x_train=np.array(wndws) :اگر آخر ردیف ها شماره کلاس نباشد
        noise_dim = noise_len
                                             # خوشه بندی پنجره ها
        ...
        for i in range (max_index):
            gnrtds=np.empty((0,smpl_rte+6), float)# تعریف اولیه آرایه تولید نمونه برای هر کلاستر بطول سیگنال+6شاخصه
            clstr=i                                                       ## تولید سیگنال برای کلاستر آی
            EPOCHS = all_smpls*int(50000/len(vars()['x_train'+str(i)]))
            if EPOCHS>5000:
                EPOCHS=5000
                epch_sv_stp=50     #dflt=10,Save Generator every 5-10-20 epoch for major-minor class
                num_examples_to_generate = 40   #dflt=100,save 100 samples per epoch
            elif 5000>EPOCHS>3000:
                epch_sv_stp=30     #dflt=10,Save Generator every 5-10-20 epoch for major-minor class
                num_examples_to_generate = 50   # dflt=100
            elif 3000>EPOCHS>1000:
                epch_sv_stp=20      #dflt=10,Save Generator every 5-10-20 epoch for major-minor class
                num_examples_to_generate = 75   # dflt=100

            mdl=int(str(all_smpls)+str(snstvty))             ## مدل با تعداد ایپاک و حساسیت به ضربان متفاوت
            ...
            print('class=',z,cluster=',i, ' >> Total generated Samples = ', len(gnrtds))
...
def make_discriminator_model():
    model = tf.keras.Sequential()
    model.add(layers.Conv1D(64, (5), strides=(2), padding='same',input_shape=[480, 1]))
    model.add(layers.LeakyReLU())
...
def train(dataset, x_trn, epochs, gnrtds, cls0, mdl0, clstr0, num_examples_to_generate, epch_sv_stp0,BATCH_SIZE, noise_dim, generator, discriminator):
    for epoch in range(epochs):
        ...
        if (epoch % epch_sv_stp0==0 and epoch>10) or ((epoch>epochs*.40)and(epoch%(epch_sv_stp0 /2)==0)) :
            generator.save('Generator_GAN_Cls_{}_mdl_{}_clstr_{}_epoch_{}_from_{}.h5'.format(cls0,mdl0,clstr0,epoch,epochs))
